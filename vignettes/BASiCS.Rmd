...
Please ensure that the acceptance rates displayed in the output of this function are around 0.44. If they are too far from this value, you might need to increase the number of iterations. 
...
<span style="color:red"> NOTE: DEFAULT PRIOR FOR GAMMA DEFINED BY LOG-NORMAL DISTRIBUTION NOW. </span> 
...
For each gene, this function returns the posterior probability as a measure of evidence for being a HVG. A cut-off value for these posterior probabilities can be set by controlling EFDR (defaul option: `EviThreshold` defined such that EFDR = 0.05).

**Note: this criteria for threshold choice is new. Original version defined  `EviThreshold` such that EFDR = EFNR.**
...
The results for the test are stored in `HVG$Table`. HVG are marked as `HVG$Table$HVG == TRUE`.

```{r quick-start-HVGdetectionTable}
head(HVG$Table)
```

<span style="color:red"> TO-DO: WE MIGHT WANT A LESS CONSERVATIVE EFDR GOAL. CHECK THIS WITH REAL DATA. </span> 

### Detection of lowly variable genes

To identify **lowly variable genes (LVG) ** within a group of cells, we follow a similar procedure. In the example below, LVG are defined as those genes for which **at most** 40\% of their total variability is attributed to the biological variability component. 

```{r quick-start-LVGdetection}
par(mfrow = c(1,2))
LVG <- BASiCS_DetectLVG(Data, Chain, VarThreshold = 0.4, Plot = TRUE)
```

The results for the test are stored in `LVG$Table`. LVG are marked as `LVG$Table$LVG == TRUE`.

```{r quick-start-LVGdetectionTable}
head(LVG$Table)
```
...
**Note: previous versions of BASiCS used natural logarithm instead**
...
```{r quick-start-DV2, results='hide', eval = FALSE}
...
plot(MCMC_Summary, Param = "mu", Param2 = "delta", log = "x", smooth = TRUE)
```

The option `smooth = TRUE` is generally recommended as this plot will contain thousands of genes when analysing real datasets. 
