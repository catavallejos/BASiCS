...
expression variability in terms of:
  ...
  1. **Over-dispersion** --- a measure for the residual cell-to-cell 
  variation that is observed after accounting for technical noise. This feature 
  has led, for example, to novel insights in the context of immune cells across 
  aging [3]. Due to the confounding effect that is typically observed 
  
  2. **Residual over-dispersion** 
  
  
More recently, BASiCS has been extended to account for 

In both cases, a probabilistic output is provided, with a decision rule that is
calibrated through the expected false discovery rate (EFDR) [4].

The original implementation of BASiCS [1] requires the use of **spike-in** 
molecules --- that are artificially introduced to each cell's lysate 
--- to perform these analyses. However, the current release of BASiCS has been
extended to also address experiments where spikes-in are not available. 
...
This section introduces the main functionality of BASiCS which include the generation
of a `r Biocpkg("SingleCellExperiment")` object prior ro running the MCMC sampler. 
To generate the `SingleCellExperiment` object, it requires a matrix of raw 
transcript counts `Counts` (columns: cells, rows: genes) 
after quality control (e.g. removing low quality cells) and filtering of lowly 
expressed genes. A logical vector `Tech` indicates which row contains technical
spike-in molecules and a data.frame object `SpikeInfo` contains the names of
the spike-in molecules in the first column and the absolute number of molecules 
per well in the second column. Section \@ref(different-sampling-modes) provides 
details on the case when technical spike-in molecules are not available.

To run the MCMC sampler, a `SingleCellExperiment` object is required as well as 
an input for the total number of iterations `N`, a thinning constant `Thin` and
the burn-in period `Burn`. For performing a regression between mean and over-dispersion
values, the paramter `Regression=TRUE` can be added (see section \@ref(different-sampling-modes)).

To perform differential testing (mean and variance), two `Chain` objects are 
required. These are obtained by running the sampler on cells from two conditions
separately.

```{r, eval=FALSE}
Data <- newBASiCS_Data(Counts, Tech, SpikeInfo)

Chain <- BASiCS_MCMC(Data = Data, N = 40000, Thin = 20, Burn = 20000)
...
```{r quick-start-HVGdetection, fig.height = 16, fig.width = 16}
par(mfrow = c(2,2))
...
This section provides details on sampling strategies when technical spike-in 
molecules are not present or when adjusting the mean confounding effect
for testing differential variability.

## No-spikes version

When technical spike-in genes are not available, BASiCS can be run via the 
horizontal integration approach. 

## Regression version

To obtain residual over-dispersion estimates that do not correlate to mean expression
parmaters, the following code can be used.
...
Additional parameters for this sampler include: `k` number of regression components
(`k`-2 radial basis functions, one intercept and one linear component), `Var` the scale
parameter influencing the width of the basis functions and `eta` the degrees of freedom.

To visualize the fit between over-dispersion $\delta_i$ and mean expression $\mu_i$
...
The `BASiCS_TestDE` test function will automatically perform differential variability 
testing on the residual over-dispersion parameters $\epsilon_i$ when providing two `Chain` objects that were generated by BASiCS' regression case.
...
Test <- BASiCS_TestDE(Chain1 = ChainSCReg, Chain2 = ChainRNAReg,
                      ...
                      EFDR_M = 0.10, EFDR_D = 0.10,
                      Offset = TRUE, OffsetPlot = FALSE, Plot = FALSE)
```

This test function outputs an extra slot containing the results for testing 
differences in residual over-dispersion. Only genes that are expressed in at least 
2 cells are included in the test. By performing the regression, all genes 
can be tested for changes in expression variability.

```{r}
head(Test$TableResDisp)
```

## No-spike and regression version

The following code chunks explain the usage of sampling when no technical spike-in
genes are available and when regression between over-dispersion and mean expression
...
```{r OtherHPD, fig.width = 16, fig.height = 16}
par(mfrow = c(2,2))
...
[7] Roberts GO and Rosenthal JS (2009). Examples of adaptive MCMC. 
*Journal of Computational and Graphical Statistics* 18: 349-367. 

[8] Eling N, Richard AC, Richardson S, Marioni JC and Vallejos CA (2017).
