...
1. **For a single group of cells** [1]: BASiCS provides a criterion to identify 
...
expression variability in terms of: 
  a. **Over-dispersion** [2] --- a measure for the excess of cell-to-cell 
  variability (with respect to Poisson sampling) that is observed after 
  accounting for technical noise. This feature has led, for example, to novel 
  insights in the context of immune cells across aging [3]. Due to the strong 
  mean/over-dispersion confounding that is typically observed for scRNA-seq 
  experiments, the assessment of changes in over-dispersion is restricted to 
  genes for which mean expression does not change between the groups.
  b. **Residual over-dispersion** [8] --- a residual measure of variability  
  given by departures with respect to a global mean/over-dispersion trend. 
  Positive residual over-dispersion indicates that a gene exhibits more 
...
implementation of BASiCS [1] requires the use of **spike-in** 
...
As a default, the code above runs 

For performing a regression between mean and over-dispersion
values, the paramter `Regression=TRUE` can be added (see section \@ref(alternative-implementation-modes)).
