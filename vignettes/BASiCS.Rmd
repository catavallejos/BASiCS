...
expression variability in terms of: 
  ...
  cell-to-cell variability (with respect to Poisson sampling) that is observed 
  ...
  [@martinez2017aging]. Due to the strong mean/over-dispersion confounding that 
  is typically observed for scRNA-seq experiments, the assessment of changes in 
  over-dispersion is restricted to genes for which mean expression does not 
  change between the groups.
  b. **Residual over-dispersion** [@eling2017robust] --- a residual measure of 
  ...
  negative esidual over-dispersion suggests less variation than expected.

In all cases, a probabilistic output is provided, with a decision rule that is
calibrated through the expected false discovery rate (EFDR) [@newton2004detecting].

A brief description for the statistical model implemented in BASiCS is 
provided in Section \@ref(methodology) of this document. While the original 
...
extended to also address experiments for which spikes-ins are not available 
(see Section \@ref(alternative-implementation-modes)).
...
To use the no-spikes implementation, please add `WithSpikes = FALSE` as an 
additional parameter. To use the regression implementation, please add 
`Regression = TRUE` as an additional parameter. 
...
Note: the `r Biocpkg("scater")` package provides enhanced functionality for the 
pre-processing of scRNA-seq datasets. 
...
The genes for which mean expression changes are marked as `ExcludedFromTesting`
in the `Test$TableDisp$ResultDiffDisp` slot.
...
Beyond is original implementation, BASiCS has been extended to adress 
...
over-dispersion for scRNA-seq datasets [@eling2017robust]. Alternative 
implementation modes are summarised below:

<img src="./AlternativeModels.png" height="400px" width="600px" />

As a default, the `BASiCS_MCMC` function uses `WithSpikes = TRUE` and 
`Regression = FALSE`.
...
replicates. Therefore, `BASiCS_MCMC` will fail to run if a single batch of
samples is provided. **Note:** batch information must be provided via the 
`BatchInfo` argument when using the `newBASiCS_Data` function. Additionally,
when creating the `BASiCS_Data` object, the `SpikeInfo` slot has to be set as
`SpikeInfo = NULL`.
...
The *regression* implementation of BASiCS uses an informative prior formulation 
to account for the relationship between mean and over-dispersion gene-specific 
parameters. The latter is used to infer a global trend between these parameters
and, subsequently, to derive a *residual over-dispersion* measure that is 
derived by departures with respect to this trend.
...
testing on the residual over-dispersion parameters $\epsilon_i$ when providing 
two `Chain` objects that were generated by BASiCS' regression case.
...
differences in residual over-dispersion. Only genes that are expressed in at 
least 2 cells are included in the test. Genes that are not expressed in at 
least 2 cells per condition are marked as `ExcludedFromRegression` in the 
`Test$TableResDisp$ResultDiffResDisp` slot. By performing the regression, 
...
**Note:** Additional parameters for this sampler include: `k` number of regression components
...
parameters (mean and over-dispersion) between the groups. 

More details regarding the model setup, prior specification and implementation 
are described in [@vallejos2015basics] and [@vallejos2016beyond]. 
...
Jens Preu&szlig;ner (jenzopr), Joanna Dreux (Joannacodes), 
Kevin Rue-Albrecht (kevinrue), Luke Zappia (lazappi), Nitesh Turaga (nturaga),
Mike Morgan (MikeDMorgan), Muad Abd El Hay (Cumol), Simon Anders (s-andrews), 
