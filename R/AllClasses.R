...
#' @name BASiCS_MultiChain
#' @aliases BASiCS_MultiChain-class
#'
#' @title The BASiCS_MultiChain class
#'
#' @description Container of several sets of MCMC samples of the BASiCS' model
#' parameters as generated by the functions \code{\link[BASiCS]{BASiCS_MCMC}}
#' or \code{\link[BASiCS]{BASiCS_MultiMCMC}}.
#' @slot chains List of \linkS4class{BASiCS_Chain} objects.
#' @examples
#'
#' # A BASiCS_Chain object created by the BASiCS_MCMC function.
#' Data <- makeExampleBASiCS_Data()
#'
#' ## run 4 chains
#' Chains <- BASiCS_MCMC(Data, N = 100, Thin = 2, Burn = 2, Regression = TRUE, NChains = 4)
#' ## runs 4 chains by default
#' Chains <- BASiCS_MultiMCMC(Data, N = 100, Thin = 2, Burn = 2, Regression = TRUE)
#'
#'
#' @export
setClass(
  "BASiCS_MultiChain",
  representation = representation(chains = "list"),
  contains = "Versioned",
  validity = function(object) {
    for (i in seq_along(object@chains)) {
      if (!inherits(object@chains[[i]], "BASiCS_Chain")) {
        stop("Chain", i, "is not a BASiCS_Chain object")
      }
    }
  },
  prototype = prototype(
    new(
      "Versioned",
      versions = c(
        "BASiCS_MultiChain" = utils::packageVersion("BASiCS")
      )
    )
  )
)
