...
HiddenThresholdSearchTestDE <- function(ChainLFC, Epsilon, EviThreshold, GenesSelect, EFDR, Task) {
    # Calculating posterior probabilities
    if (Epsilon > 0) {
        Prob = HiddenProbDE(chain = abs(ChainLFC), tol = Epsilon)
    } else {
    ...
    list(Prob = Prob, OptThreshold = OptThreshold, EFDRgrid = EFDRgrid, EFNRgrid = EFNRgrid)
}
