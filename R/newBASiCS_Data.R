...
newBASiCS_Data <- function(Counts, Tech, SpikeInfo, BatchInfo = NULL) {
    ...
    if (!is.null(SpikeInfo)) {
        ...
        if (sum(!(SpikeInfo[, 1] %in% GeneNames[Tech])) > 0) 
            stop("'SpikeInfo' includes spikes that are not in the Counts matrix")
    ...
    if (sum(Counts%%1) > 0) 
        errors <- c(errors, "Invalid value for 'Counts' (entries must be positive integers)")
    if (!(is.logical(Tech))) 
        errors <- c(errors, "Invalid value for 'Tech'")
    if (!(is.numeric(SpikeInput) & all(SpikeInput > 0) & sum(!is.finite(SpikeInput)) == 0)) 
    ...
    if (length(errors) == 0) 
        TRUE else stop(errors)
    
    # Create a SingleCellExperiment data object
    Data <- SingleCellExperiment::SingleCellExperiment(assays = list(Counts = as.matrix(Counts)), metadata = list(SpikeInput = SpikeInput, 
        BatchInfo = BatchInfo))
