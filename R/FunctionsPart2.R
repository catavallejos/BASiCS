#' @title Creates a BASiCS_DV_Data object from expression counts matrices and experimental information about spike-in genes
#' 
#' @description \code{newBASiCS_DV_Data} creates a \code{\link[BASiCS]{BASiCS_DV_Data-class}} object from 
...
#' 
#' @return An object of class \code{\link[BASiCS]{BASiCS_DV_Data-class}}.
#' 
#' @examples
#' 
...
newBASiCS_DV_Data <- function(CountsTest, 
                              CountsRef, 
                              Tech, 
                              SpikeInputTest, 
                              SpikeInputRef){
  Data <- new("BASiCS_DV_Data", CountsTest = CountsTest, CountsRef = CountsRef, Tech = Tech, 
              SpikeInputTest = SpikeInputTest, SpikeInputRef = SpikeInputRef)
  show(Data)
  cat('\n')
  cat('NOTICE: BASiCS_D requires a pre-filtered dataset \n')
  ...
  cat('\n')
  cat(' BASiCS_DV_Filter can be used for this purpose \n')
  return(Data)  
}

#' @name makeExampleBASiCS_DV_Data
#' 
#' @title Create a simple example of a BASiCS_DV_Data object with random data
#' 
#' @description A simple \code{\link[BASiCS]{BASiCS_DV_Data-class}} object is generated by simulating a dataset from the
#' model in BASiCSDV. This is used for the examples in the package and the vignette. 
#' 
#' @return An object of class \code{\link[BASiCS]{BASiCS_DV_Data-class}}, simulated from the model implemented in BASiCSDV. 
#' It contains 120 genes (100 biological and 20 spike-in) and 20 cells (10 in each sample). 
#' 
#' @examples 
#' Data = makeExampleBASiCS_DV_Data()
#' is(Data, "BASiCS_DV_Data")
#' 
#' @author Catalina A. Vallejos \email{catalina.vallejos@@mrc-bsu.cam.ac.uk}
makeExampleBASiCS_DV_Data <- function()
{
  ...
  Data = newBASiCS_DV_Data(CountsTest = Counts.sim[,1:n_test], 
...
#' @title Creates a BASiCS_DV_Chain object from pre-computed MCMC chains
#' 
#' @description \code{BASiCS_DV_Chain} creates a \code{\link[BASiCS]{BASiCS_DV_Chain-class}} object from pre-computed MCMC chains.
#' 
...
#' 
#' @return An object of class \code{\link[BASiCS]{BASiCS_DV_Chain-class}}. 
#' 
#' @examples
#' 
#' # Data = makeExampleBASiCS_DV_Data()
#' # MCMC_Output <- BASiCS_DV_MCMC(Data, N = 50, Thin = 5, Burn = 5, 
...
#' 
#' # MCMC_Output_Load <- newBASiCS_DV_Chain(muTest = ChainMuTest, 
#' #                                        muRef = ChainMuRef, 
...
#' 
#' @author Catalina A. Vallejos \email{catalina.vallejos@@mrc-bsu.cam.ac.uk}
newBASiCS_DV_Chain <- function(muTest, muRef, deltaTest, deltaRef, 
                               phi, s, nu, thetaTest, thetaRef)
{
  Chain <- new("BASiCS_DV_Chain", muTest = muTest, muRef = muRef, 
               deltaTest = deltaTest, deltaRef = deltaRef, 
               phi = phi, s = s, nu = nu, 
               thetaTest = thetaTest, thetaRef = thetaRef)
  show(Chain)
  return(Chain)  
}
...
#' @author Catalina A. Vallejos \email{catalina.vallejos@@mrc-bsu.cam.ac.uk}
#' 
#' @rdname BASiCS_DV_TestDE
BASiCS_DV_TestDE <- function(Data, 
                             object,
                             GeneNames,
                             EpsilonM = 0.10,
                             EpsilonD = 0.10,
                             ...
                             ...)
{
  ...
  if(!is(Data,"BASiCS_DV_Data")) stop("'Data' is not a 'BASiCS_DV_Data' class object.")    
  if(!is(object,"BASiCS_DV_Chain")) stop("'object' is not a 'BASiCS_DV_Chain' class object.")    
