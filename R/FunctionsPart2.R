...
#' Data = newBASiCS_D_Data(CountsTest, CountsRef, Tech, SpikeInputTest, SpikeInputRef)
#' 
...
#' 
#' @return An object of class \code{\link[BASiCS]{BASiCS_D_Data-class}}, simulated from the model implemented in BASiCSDV. 
                             ...
                             ...)
{
  ...
  if(is.null(EviThresholdM))
  {
    ...
    #      if(length(optimal)>0){OptThresholdM <- c(EviThresholds[optimal], EFDR[optimal], EFNR[optimal])}
    #      else 
    #      {
    ...
    #        else
    #        {
    #          cat("Numerical issues when computing EFDR and EFNR. Please try a different tolerance for fold changes in expression")
    #          OptThresholdM <- rep("Not found",3) 
    #        }
    #      optimal <- which(EFDR - )
    #      }
  ...
  if(is.null(EviThresholdD))
  {
    ...
    #    above <- EFDR > EFNR
    #    optimal<-which(diff(above)!=0)
    #    if(length(optimal)>0){OptThresholdD <- c(EviThresholds[optimal], EFDR[optimal], EFNR[optimal])}
    #    else 
    #    {
    #      print("It is not possible to find an optimal evidence threshold for the given variance contribution threshold. \n")
    #      optimal <- round(median(which(abs(EFDR - EFNR) == min(abs(EFDR - EFNR), na.rm = T))))
  ...
  #  if(OptThresholdM[1] == "Not found") {HVG = rep("Not found", length(Sigma))}
