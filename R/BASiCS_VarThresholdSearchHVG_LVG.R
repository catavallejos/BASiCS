...
#' @param EFDR Target for expected false discovery rate related to HVG/LVG detection. 
#' Default: \code{EFDR = 0.10}.
...
#' highly variable genes detection for different variance contribution thresholds.}
#' \item{\code{BASiCS_VarThresholdSearchLVG}}{A table displaying the results of 
#' lowly variable genes detection for different variance contribution thresholds.}
#' }
#'
#'
...
BASiCS_VarThresholdSearchHVG = function(Chain, 
    ...
    if (sum(VarThresholdsGrid < 0) > 0 | sum(VarThresholdsGrid > 1) > 0 | 
    ...
    for (i in 1:length(VarThresholdsGrid)) {
        ...
        if (Progress) {
            message(paste0("Evaluating variance contribution threshold = ", 
                           100 * VarThreshold, " % ... \n"))
        }
        ...
        Table[i, ] = c(100 * VarThreshold, round(100 * DetectHVG$EFDR, 2), 
                       round(100 * DetectHVG$EFNR, 2), DetectHVG$EviThreshold, 
...
BASiCS_VarThresholdSearchLVG = function(Chain, 
    ...
    for (i in 1:length(VarThresholdsGrid)) {
        ...
        Table[i, ] = c(100 * VarThreshold, round(100 * DetectLVG$EFDR, 2), 
                       round(100 * DetectLVG$EFNR, 2), DetectLVG$EviThreshold, 
