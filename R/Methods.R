...
setMethod("show", signature = "BASiCS_Chain", definition = function(object) {
    N = nrow(object@mu)
    ...
    if (nBatch > 1) {
        ...
        message(" Dataset contains ", q.bio, " biological genes and ", n, " cells (1 batch). \n", sep = "")
    }
...
#' 
#' Vallejos, Marioni and Richardson (2015). Bayesian Analysis of Single-Cell Sequencing data. PLoS Computational Biology.
setMethod("Summary", signature = "BASiCS_Chain", definition = function(x, prob = 0.95) {
    ...
    Output <- new("BASiCS_Summary", mu = cbind(Mu, HPDMu), delta = cbind(Delta, HPDDelta), phi = cbind(Phi, HPDPhi), 
        s = cbind(S, HPDS), nu = cbind(Nu, HPDNu), theta = cbind(Theta, HPDTheta))
    return(Output)
})
...
setMethod("plot", signature = "BASiCS_Chain", definition = function(x, Param = "mu", Gene = NULL, Cell = NULL, Batch = 1, 
    ...
    if (Param %in% c("mu", "delta") & is.null(Gene)) 
    ...
    if (Param == "mu") {
        ...
        Column = Gene
        if (ylab == "") 
            ylab = bquote(mu[.(Column)])
    }
    if (Param == "delta") {
        object = x@delta
        Column = Gene
        if (ylab == "") 
            ylab = bquote(delta[.(Column)])
    }
    if (Param == "phi") {
        object = x@phi
        Column = Cell
        if (ylab == "") 
            ylab = bquote(phi[.(Column)])
    }
    if (Param == "s") {
        object = x@s
        Column = Cell
        ylab = if (ylab == "") 
            ylab = bquote(s[.(Column)])
    }
    if (Param == "nu") {
        object = x@nu
        Column = Cell
        ylab = if (ylab == "") 
            ylab = bquote(nu[.(Column)])
    }
    if (Param == "theta") {
        object = x@theta
        Column = Batch
        ylab = if (ylab == "") 
            ylab = bquote(theta[.(Column)])
    }
    
    par(mfrow = c(1, 2))
    plot(object[, Column], type = "l", xlab = xlab, ylab = ylab, main = colnames(object)[Column], ...)
...
#' 
#' Vallejos, Marioni and Richardson (2015). Bayesian Analysis of Single-Cell Sequencing data. PLoS Computational Biology.
setMethod("displayChainBASiCS", signature = "BASiCS_Chain", definition = function(object, Param = "mu") {
...
setMethod("show", signature = "BASiCS_Summary", definition = function(object) {
    q = nrow(object@mu)
    ...
    if (nBatch > 1) {
        message(" Dataset contains ", q, " genes (", q.bio, " biological and ", q - q.bio, " technical) and ", n, 
            " cells (", nBatch, " batches). \n", sep = "")
    } else {
        message(" Dataset contains ", q, " genes (", q.bio, " biological and ", q - q.bio, " technical) and ", n, 
            " cells (1 batch). \n", sep = "")
    }
...
#' Vallejos, Marioni and Richardson (2015). Bayesian Analysis of Single-Cell Sequencing data. PLoS Computational Biology.
#'
setMethod("plot", signature = "BASiCS_Summary", definition = function(x, Param = "mu", Param2 = NULL, Genes = NULL, 
    Cells = NULL, Batches = NULL, xlab = "", ylab = "", xlim = "", ylim = "", pch = 16, col = "blue", bty = "n", 
    SmoothPlot = TRUE, ...) {
    ...
    if (is.null(Param2)) {
        ...
        if (SmoothPlot) {
            col = grDevices::rgb(grDevices::col2rgb(col)[1], grDevices::col2rgb(col)[2], grDevices::col2rgb(col)[3], 
        ...
        if (Param == "mu" & Param2 == "delta") {
            ...
            if (xlim == "") {
                xlim = c(min(x@mu[Columns, 1]), max(x@mu[Columns, 1]))
            }
            if (ylim == "") {
                ylim = c(min(x@delta[Columns, 1]), max(x@delta[Columns, 1]))
            }
            
            plot(x@mu[Columns, 1], x@delta[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
        if (Param == "delta" & Param2 == "mu") {
            ...
            if (ylim == "") {
                ylim = c(min(x@mu[Columns, 1]), max(x@mu[Columns, 1]))
            }
            if (xlim == "") {
                xlim = c(min(x@delta[Columns, 1]), max(x@delta[Columns, 1]))
            }
            plot(x@delta[Columns, 1], x@mu[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
        if (Param == "phi" & Param2 == "s") {
            ...
            if (ylab == "") 
                ylab = expression(s[i])
            if (xlab == "") 
                xlab = expression(phi[i])
            if (xlim == "") {
                xlim = c(min(x@phi[Columns, 1]), max(x@phi[Columns, 1]))
            }
            if (ylim == "") {
                ylim = c(min(x@s[Columns, 1]), max(x@s[Columns, 1]))
            }
            plot(x@phi[Columns, 1], x@s[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
        if (Param == "s" & Param2 == "phi") {
            ...
            if (xlab == "") 
                xlab = expression(s[i])
            if (ylim == "") {
                ylim = c(min(x@phi[Columns, 1]), max(x@phi[Columns, 1]))
            }
            if (xlim == "") {
                xlim = c(min(x@s[Columns, 1]), max(x@s[Columns, 1]))
            }
            plot(x@s[Columns, 1], x@phi[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
        if (Param == "phi" & Param2 == "nu") {
            ...
            if (ylim == "") {
                ylim = c(min(x@nu[Columns, 1]), max(x@nu[Columns, 1]))
            }
            plot(x@phi[Columns, 1], x@nu[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
        if (Param == "nu" & Param2 == "phi") {
            ...
            if (xlim == "") {
                xlim = c(min(x@nu[Columns, 1]), max(x@nu[Columns, 1]))
            }
            plot(x@nu[Columns, 1], x@phi[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
        if (Param == "s" & Param2 == "nu") {
            ...
            plot(x@s[Columns, 1], x@nu[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
        if (Param == "nu" & Param2 == "s") {
            ...
            plot(x@nu[Columns, 1], x@s[Columns, 1], xlab = xlab, ylab = ylab, ylim = ylim, pch = pch, col = col, 
                bty = bty, ...)
        }
...
#' 
#' Vallejos, Marioni and Richardson (2015). Bayesian Analysis of Single-Cell Sequencing data. PLoS Computational Biology.
setMethod("displaySummaryBASiCS", signature = "BASiCS_Summary", definition = function(object, Param = "mu") {
